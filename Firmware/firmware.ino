#define _ARDUINO_SPOT_WELDER_V3_3_5
/***********************************************************************************************//**
 ...
 *  \par
 *              You need to have the Adafruit_GFX library and the Adafruit_SSD1306 library
 ...
 *              Adafruit GFX V1.12.0 
 *
 *  \file       Arduino_Spot_Welder_V3_3_5.ino
 *  \author     Marc Schönfleisch <http://malectrics.eu/>
 *  \version    3.3.5
 *  \date       April 2025
 *  \copyright  Copyright(c)2025 Marc Schönfleisch <info@malectrics.eu>. All right reserved.
 *
 *  \par        Changelog
 ...
 *  \li         01apr2020  RS fix 10.000 welds count limit
 *  \li         03apr2020  MS add overvoltage alarm
 *  \li         09apr2025  MS improve measured voltage offset calibration
 *
 *  \par        License
...
#include <EEPROM.h>
#include "printf.h"
#include "Arduino_Spot_Welder_V3_3_5.h"
...
uint8_t rawbatteryVoltage = DEF_NOM_BATT_V;    /**< Current battery voltage x10, raw measured without offset */
...
void stateMachine() {
	...
	// this state cannot be exited by clicking the rotary encoder
	case ST_BATTERY_LOW:
      ...
      // Overvoltage battery state. Entered by receiving a high battery event,
  ...
  case ST_BATTCAL_SCREEN: 
    ...
    static uint16_t batteryVoltageSnapshot = 0;


    if (!introShown) {
			  checkRawBatteryVoltageInstant();
        batteryVoltageSnapshot = rawbatteryVoltage;  // <– freeze the raw battery voltage value during calibration
        measuredVoltage = batteryVoltageSnapshot + pData.batteryOffset;
        displayThreeLines("Battery Calibration",
                          "Set: rotate encoder",
                          "Save: press encoder");
        introShown = true;
        changed = false;
    ...
    if (mEvent == EV_ENCUP || mEvent == EV_ENCDN) {
        ...
        if (mEvent == EV_ENCDN)
            measuredVoltage = measuredVoltage > MIN_BATT_V ?
                              measuredVoltage - 1 : MIN_BATT_V;
        else
            measuredVoltage = measuredVoltage < MAX_BATT_V ?
                              measuredVoltage + 1 : MAX_BATT_V;

        pData.batteryOffset = measuredVoltage - batteryVoltageSnapshot;

        char buf[10], line1[20], line2[20], line3[20];

        // Target Voltage
        valStr(buf, measuredVoltage, VF_BATTV);
        snprintf(line1, sizeof(line1), "Target: %sV", buf);

        // measured raw voltage (freezed during calibration)
        valStr(buf, batteryVoltageSnapshot, VF_BATTV);
        snprintf(line2, sizeof(line2), "Real:   %sV", buf);

        // Offset-Value
        int16_t diff = measuredVoltage - batteryVoltageSnapshot;
        valStr(buf, abs(diff), VF_BATTV);
        snprintf(line3, sizeof(line3), "Offset: %c%sV", (diff >= 0 ? '+' : '-'), buf);

        displayThreeLines(line1, line2, line3);
    }

    else if (mEvent == EV_BTNDN) {
        

        char buf[10], msg[20];
        valStr(buf, abs(pData.batteryOffset), VF_BATTV);
        snprintf(msg, sizeof(msg), "Offset: %c%sV",
                 (pData.batteryOffset >= 0 ? '+' : '-'), buf);

        displayThreeLines("Calibration saved", "New offset:", msg);
        delay(1500);
...
void checkRawBatteryVoltageInstant() {
  
  rawbatteryVoltage = (uint8_t)( analogRead( PIN_BATT_V ) * 5.0 * 4.03 * 10.0 /
                                1023.0 );
}
...
void checkForLowVoltageEvent() {
	...
	if ( millis() - lastBVTime > BV_INTERVAL ) {
      ...
      // If the battery voltage is to high then issue a high voltage event.
...
//this will make the voltage and amps reading 0 if the voltage druing pulse is above 10V. 100 equals 10.0V measured voltage,                                         
...
/**
 *  \brief  Helpder function for 3 Lines of info on the display.
 */
void displayThreeLines(const char* line1, const char* line2, const char* line3) {
    display.clearDisplay();
    display.setTextSize(1);       // kleiner Font
    display.setTextColor(WHITE);
    display.setCursor(0, 16);     // vertikales Offset (zentriert auf 64px)
...
void drawStatusLine() {
	...
	setTextProp( 1, 8 * CHR_W, 1 );
  	display.print( valStr( str, pData.weldCount, VF_WELDCNT ) ); //Shows weld count in the status bar
//  display.print( valStr( str, batteryVoltage, VF_BATTV ) ); // Shows voltage instead of weld counts in the status bar
	setTextProp( 1, 13 * CHR_W, 1 );
  display.print( FPSTR( LS_WELDS ) );  //Shows weld count in the status bar
//display.print( FPSTR( LS_VUNITS ) ); // Shows voltage instead of weld counts in the status Bar
